import { TransactionFeatures, FraudModelMetadata, FraudSeverity } from '../types/fraud';
...
export class LstmAnomalyDetector {
  ...
  train(historicalData: TransactionFeatures[]): void {
    ...
    logger.info(`LSTM Autoencoder retrained. New anomaly threshold: ${this.normalThreshold.toFixed(4)}`);
  }
}
...
export class XgboostWashTradingClassifier {
  async predict(features: TransactionFeatures): Promise<InferenceResult> {
    ...
    const hasVolumeClustering = features.rollingTxCount > 40 && Math.abs(features.priceCorrelation) > 0.4;
...
export class ExploitPredictor {
  async predict(features: TransactionFeatures): Promise<InferenceResult> {
    ...
    // Exploit indicators
    const isFlashLoan = features.contractCallDepth > 4 && features.storageAccessPatterns.writes > 15;
    const isOracleManip = Math.abs(features.priceCorrelation) > 0.8 && features.dexLiquidityChange > 0.5;
    const isReentrancy = features.contractCallDepth > 6 && features.storageAccessPatterns.reads > 25;

    const exploitProbability =
      (isFlashLoan ? 0.5 : 0.0) +
      (isOracleManip ? 0.4 : 0.0) +
      (isReentrancy ? 0.7 : 0.1);
