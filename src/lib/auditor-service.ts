...
export async function verifyAuditor(
  ...
  return { ok: true, message: `Auditor "${auditor.name}" verified. Badge: ${badge ?? 'none yet'}.` };
}
...
export async function updateAuditorTrustScore(
  auditorId: string,
  accepted:  boolean,
): Promise<void> {
  const auditor = await prismaRead.auditorRegistry.findUnique({
    where:  { id: auditorId },
    select: { trustScore: true, totalAudits: true, acceptedAudits: true, rejectedAudits: true, isVerified: true },
  });
  if (!auditor) return;
...
export async function submitExternalAudit(
  ...
  const auditor = await prismaRead.auditorRegistry.findFirst({
    ...
    select: {
      id: true, name: true, verificationKey: true,
  ...
  if (keyToUse && input.reportHash && input.reportSignature) {
    signatureVerified = verifyReportSignature(
      input.reportHash,
      input.reportSignature,
      keyToUse,
    );
  }
