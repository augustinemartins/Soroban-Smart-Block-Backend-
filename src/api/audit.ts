...
const VALID_CATEGORIES = ['token','dex','nft','lending','staking','bridge','governance','other'] as const;
...
auditRouter.get('/benchmarks', async (_req: Request, res: Response) => {
  try {
    ...
    const avgOverall       = totalContracts > 0
...
auditRouter.get('/stats', async (_req: Request, res: Response) => {
  try {
    ...
    const resolutionRate = totalOpen + totalResolved > 0
    ...
    const result = {
      ...
      platform: {
        ...
        scoreWeights:        { security: '30%', governance: '25%', economic: '20%', compliance: '15%', liquidity: '10%' },
      },
    };
...
auditRouter.get('/stats/by-contract', async (req: Request, res: Response) => {
  try {
    ...
    const orderBy = q.sort === 'score_asc'  ? { overallScore: 'asc'  as const } :
                    q.sort === 'score_desc' ? { overallScore: 'desc' as const } :
                    q.sort === 'findings'   ? { criticalFindings: 'desc' as const } :
    ...
    const [certs, total] = await Promise.all([
      prismaRead.auditCertificate.findMany({
        ...
        select: {
          ...
          economicScore: true, complianceScore: true, liquidityScore: true,
          totalFindings: true, criticalFindings: true, highFindings: true,
          mediumFindings: true, lowFindings: true, openFindings: true,
          certificateHash: true, anchorTxHash: true, generatedAt: true,
        },
      }),
      prismaRead.auditCertificate.groupBy({
        by:    ['contractAddress'],
...
auditRouter.get('/stats/trends', async (req: Request, res: Response) => {
  try {
    ...
    const certs = await prismaRead.auditCertificate.findMany({
      ...
      select: {
        generatedAt: true, overallScore: true,
    ...
    for (const c of certs) {
      ...
      const b   = bucketsMap.get(key) ?? {
        date: key, count: 0,
        sumOverall: 0, sumSec: 0, sumGov: 0, sumEco: 0, sumCom: 0, sumLiq: 0,
        sumFindings: 0, sumCritical: 0, sumOpen: 0,
      };
      b.count++;
...
const ALERT_TYPES = ['score_drop', 'new_finding', 'upgrade', 'certificate_update', 'certificate_expiry'] as const;
...
auditRouter.post('/subscriptions', async (req: Request, res: Response) => {
  try {
    ...
    const contract = await prismaRead.contract.findUnique({
      where: { address: data.contractAddress }, select: { address: true },
    });
    if (!contract) {
      return res.status(404).json({ error: 'Contract not found in explorer index.' });
    }
...
auditRouter.get('/subscriptions/:id', async (req: Request, res: Response) => {
  try {
    ...
    const deliveries = await prismaRead.auditNotificationDelivery.findMany({
      ...
      take:    10,
      select: {
        id: true, alertType: true, channel: true, status: true,
        httpStatus: true, deliveredAt: true, errorMsg: true, createdAt: true,
      },
    });
