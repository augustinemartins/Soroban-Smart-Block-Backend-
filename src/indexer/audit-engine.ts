...
async function scoreSecurityDimension(contractAddress: string): Promise<{
  ...
  const [reentrancyAlerts, mevEvents, freezeViolations, threatAdvisories, verifyJob] =
    await Promise.all([
      ...
      prismaRead.threatAdvisory.findMany({
        ...
        select: { id: true, title: true, severity: true, cvssScore: true, mitigations: true, cveId: true },
      }),
  ...
  for (const adv of threatAdvisories) {
    const sev: FindingSeverity =
      adv.severity === 'critical' || adv.severity === 'high' || adv.severity === 'medium' ||
      adv.severity === 'low' || adv.severity === 'info'
  ...
  return {
    ...
    detail: {
      ...
      wasmHash: (await prismaRead.contract.findUnique({
        where: { address: contractAddress }, select: { wasmHash: true },
      }))?.wasmHash ?? null,
    },
  };
}
...
async function scoreGovernanceDimension(contractAddress: string): Promise<{
  ...
  if (latest?.governanceType === 'single_key') {
    ...
    findings.push({
      ...
      description: 'Contract upgrades are controlled by a single private key with no multisig or timelock.',
  ...
  if (upgrades.length > 0 && (!latest?.timelockSeconds || latest.timelockSeconds === 0)) {
    ...
    findings.push({
      ...
      description: 'Upgrades can be executed immediately with no enforced delay for community review.',
  ...
  if (suspiciousCount > 0) {
    ...
    findings.push({
      ...
      detail: `Flags: ${upgrades.filter((u) => u.isSuspicious).flatMap((u) => u.suspiciousFlags).join(', ')}`,
...
async function scoreEconomicDimension(contractAddress: string): Promise<{
  ...
  const [txStats, sandwichCount, yieldOpp, portfolio, prevPortfolio] = await Promise.all([
    ...
    prismaRead.yieldOpportunity.findFirst({
      where: { contractAddress }, orderBy: { updatedAt: 'desc' },
    }),
    prismaRead.portfolioSnapshot.findFirst({
      where: { contractAddress }, orderBy: { snapshotAt: 'desc' },
    }),
    prismaRead.portfolioSnapshot.findFirst({
      where: { contractAddress, snapshotAt: { lt: since30d } }, orderBy: { snapshotAt: 'desc' },
    }),
  ]);
  ...
  if (tvl30dTrend < -30) {
    deductions += 20;
    findings.push({
      category: 'economics', severity: 'high',
    ...
    deductions += 10;
    findings.push({
      category: 'economics', severity: 'medium',
      title: `TVL Declined ${Math.abs(tvl30dTrend).toFixed(1)}% Over 30 Days`,
  ...
  if (sandwichCount > 10) {
    ...
    findings.push({
      ...
      recommendation: 'Implement slippage protection, use commit-reveal, or enable private mempool routing.',
    });
  ...
  if (uniqueUsers.length < 10 && txStats._count.id > 0) {
    deductions += 10;
    findings.push({
      category: 'economics', severity: 'low',
...
async function scoreComplianceDimension(contractAddress: string): Promise<{
  ...
  const [upgrades, deployer] = await Promise.all([
    prismaRead.wasmUpgradeHistory.findMany({
      where: { contractAddress }, select: { upgrader: true },
    }),
    prismaRead.transaction.findFirst({
      where: { contractAddress }, orderBy: { ledgerSequence: 'asc' },
  ...
  if (sanctionHits.length > 0) {
    ...
    for (const hit of sanctionHits) {
      const role = hit.address === contractAddress ? 'contract' :
        hit.address === deployer?.sourceAccount ? 'deployer' : 'admin key';
      findings.push({
  ...
  const score = clamp(SCORE_MAX - deductions);
  const status = sanctionHits.length > 0 ? 'restricted' :
    highFlags.length > 0 ? 'flagged' : 'clean';
...
async function scoreLiquidityDimension(contractAddress: string): Promise<{
  ...
  if (poolLiquidity < 1000 && (dexPool || ammPool || yieldOpp)) {
    deductions += 30;
    findings.push({
      category: 'liquidity', severity: 'high',
      ...
      recommendation: 'Bootstrap liquidity via incentive programs before enabling large-volume trading.',
    });
  } else if (poolLiquidity < 50000 && poolLiquidity > 0) {
    deductions += 15;
    findings.push({
      category: 'liquidity', severity: 'medium',
  ...
  if (dexPool?.poolPrices && dexPool.poolPrices.length < 2) {
    ...
    findings.push({
      ...
      description: 'Price feed history is sparse, suggesting liquidity is concentrated in very few providers.',
  ...
  if (dexPool?.poolPrices && dexPool.poolPrices.length >= 2) {
    ...
    if (ilIndicator > 0.1) {
      ...
      findings.push({
        ...
        description: 'Recent price volatility suggests LPs may be experiencing significant impermanent loss.',
...
export function generateReportText(cert: {
  ...
  const grade =
    cert.overallScore >= 85 ? 'A' :
    cert.overallScore >= 70 ? 'B' :
    cert.overallScore >= 55 ? 'C' :
    cert.overallScore >= 40 ? 'D' : 'F';

  const riskLabel =
    cert.overallScore >= 85 ? 'LOW' :
    cert.overallScore >= 70 ? 'MEDIUM' :
    cert.overallScore >= 55 ? 'HIGH' : 'CRITICAL';
  ...
  return [
    ...
    `SIGNATURE ALG      : ed25519 (HMAC-SHA256)`,
    ...
    const cert = await prismaWrite.auditCertificate.create({
      data: {
        ...
        metadata: {
          triggerSource,
          engineVersion: '2.0.0',
          weightsUsed: { security: 0.30, governance: 0.25, economic: 0.20, compliance: 0.15, liquidity: 0.10 },
