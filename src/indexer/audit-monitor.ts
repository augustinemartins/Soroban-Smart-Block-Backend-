...
async function detectAdminChanges(): Promise<DetectedSignal[]> {
  ...
  for (const u of upgrades) {
    ...
    const severity: 'low' | 'medium' | 'high' | 'critical' =
      isCritical   ? 'critical' :
      isSuspicious ? 'high' :
      u.changeClassification === 'major' ? 'medium' : 'low';
  ...
  for (const { contractAddress } of activeContracts) {
    ...
    if (Math.abs(changePct) >= USER_DROP_PCT && curr < prev) {
      signals.push({
        ...
        detail:     { previousWeekUsers: prev, currentWeekUsers: curr, changePct: changePct.toFixed(2) },
...
export async function emitPostAuditAlerts(
  ...
  const [newCert, prevCert] = await Promise.all([
    prismaRead.auditCertificate.findUnique({
      ...
      select: {
        ...
        generatedAt: true, certificateHash: true,
      },
    }),
  ...
  const grade    = newCert.overallScore >= 85 ? 'A' : newCert.overallScore >= 70 ? 'B' :
                   newCert.overallScore >= 55 ? 'C' : newCert.overallScore >= 40 ? 'D' : 'F';
  const risk     = newCert.overallScore >= 85 ? 'low' : newCert.overallScore >= 70 ? 'medium' :
                   newCert.overallScore >= 55 ? 'high' : 'critical';
  ...
  if (prevCert) {
    ...
    if (drop >= SCORE_ALERT_DROP) {
      ...
      incidentScoreDropBelowThreshold(
        ...
        newCert.id,
        newCert.overallScore,
      ).catch((e) => logger.warn('incidentScoreDropBelowThreshold failed', { error: String(e) }));
  ...
  for (const f of newFindings) {
    ...
    if (f.severity === 'critical') {
      incidentCriticalFinding(
        ...
        f.id,
        f.title,
      ).catch((e) => logger.warn('incidentCriticalFinding failed', { error: String(e) }));
    }
...
async function runMonitorCycle(): Promise<void> {
  ...
  const [
    ...
    detectReentrancy().catch((e) => { logger.warn('Reentrancy detector error', { error: String(e) }); return []; }),
    detectMevAttacks().catch((e) => { logger.warn('MEV detector error',        { error: String(e) }); return []; }),
    detectDependencyRisk().catch((e) => { logger.warn('Dependency detector error', { error: String(e) }); return []; }),
    detectAdminChanges().catch((e) => { logger.warn('Admin change detector error', { error: String(e) }); return []; }),
    detectTvlChanges().catch((e) => { logger.warn('TVL detector error',        { error: String(e) }); return []; }),
    detectUserChanges().catch((e) => { logger.warn('User change detector error', { error: String(e) }); return []; }),
    detectSanctions().catch((e) => { logger.warn('Sanctions detector error',   { error: String(e) }); return []; }),
  ]);
  ...
  for (const addr of reauditSet) {
    const trigger =
      allSignals.find((s) => s.contractAddress === addr && s.signalType === 'admin_change')
        ? 'upgrade'
    ...
    runAuditPipeline({
      ...
      .then((result) => {
        ...
        if (trigger === 'upgrade') {
          notifyUpgrade(
            addr, result.certId, result.version,
            result.overallScore,
...
export function startContinuousAuditMonitor(): void {
  ...
  setTimeout(() => {
    runMonitorCycle().catch((e) =>
      logger.error('Audit monitor cycle error', { error: String(e) }),
    );
  }, 30_000);
